# Phase 2 — the Finnhub vertical slice

Phase 2 made one provider real, end to end. Finnhub quotes now flow: client → scheduler →
...
**Exit criterion, met:** live AAPL on screen with its honest age, and a manual trade valued
off Finnhub through the new (provider, symbol) pricing cache. Evidence in Step 8.

Reading guide: the vocabulary section below defines every term the report leans on, and
the "whole path" section after it walks the running system in order — one quote, boot, one
trade — so the steps land in a picture you already have. Steps open with *Needed* (the
problem, in plain words), then bullets of what was built and why. Indented sub-bullets are optional
deep-dives. Plan references like "(plan D7)" are breadcrumbs, never required reading. The
report describes what exists at the end of Phase 2 and nothing beyond it; what is
deliberately missing is listed at the end, and how each gap closes is the plan's business,
not this report's.

## The vocabulary — what Phase 1 left in place

Phase 1 designed the data model: it created the market tables and the code contracts, all
*empty* — nothing fetched data yet. Phase 2 is the phase that fills them. The terms:
...
- **History** — table `market_data_snapshots`: one row appended each time a price actually
  *changes*, carrying the provider's raw JSON payload. This is the audit store. The
  ...
  but the code written this phase already honors it: the retention sweep never deletes a
...
- **Freshness** — a quote's state given its age: `LIVE` (younger than its threshold),
  ...
  cannot serve this asset class at all). Phase 1 defined the states and left the
  *thresholds* open — they depend on how often each feed polls, which is decided in this
  phase: threshold = 3× the symbol's polling cadence.
...
The steps of this report each describe one component. This section is the connected story
...
3. pricing seeds its quote cache from that snapshot, then attaches to the stream — the
   ...
   restart instead of waiting for fresh polls. *(review section)*
...
2. Within 2 s, pricing's refresh loop notices the new ACTIVE trade and values it
   immediately off the already-cached quote — no waiting for the next poll. *(review
   section)*
3. Within 5 s, the blotter's reconcile (or the valuation stream, whichever is first) makes
   the trade visible in Trades/Books/Valuations — observed end to end at ≤ 4 s. *(review
   section)*
...
The business rules Phase 2 set — the policy layer, each with its why and where the
mechanics live. (The README's "Operating decisions" table digests these together with the
other phases' rules.)
...
9. **Retention: 90 days, except rows a trade references.** Provenance outlives
   housekeeping. (Shipped at 30; raised to 90 during review — chart depth vs. hosted
   database storage.) *(Step 3)*
...
- **Every request spends one token** — quotes, the market-status check, manual refreshes.
  ...
  - Why `time.monotonic()` and not `datetime.now()`: monotonic time is a stopwatch — it
    ...
    phase: budget and cooldown math on monotonic, anything stored or shown on `utcnow()`.
...
**Needed:** Phase 1 created the board and history tables (vocabulary above) and *decided*
how they should behave; this step writes the code that actually behaves that way. Three
...
- **`/snapshot` now reads the database, so restarts are warm for free.** Before this phase
  the service kept quotes in an in-process dict that died with the process. The board table
...
the budget — and react sanely when Finnhub fails. That is several distinct jobs, and after
review they live in four small files instead of one big one (owner feedback 2026-08-19:
too much for one file). The subsections: 4.1 the layout, 4.2 the symbol universe, 4.3 and
...
The loop is where this phase's parts become a running system. Steps 1, 2, 3 and 5 each
...
6. **`pollable`** filters the active set through `supports_quotes(FINNHUB, asset_class)`
   — Phase 1's capability matrix, consulted every cycle. A watchlist FX row would be
...
The happy path is `_fetch_and_publish` — six calls, and each one is a component this
phase built. Verbatim:
...
- **`stale_after_seconds(symbol)` — the freshness threshold, 3× the tier cadence.** It
  ...
  CLOSED display is a gap listed at the end of this report.
...
demo where a key dies should *show* what happened, not silently stop.
...
machinery predates this phase; the new publisher had to keep speaking it, with
...
- **A scenario-analysis bug got fixed while touching this code.** Quotes travel over SSE
  ...
  `Decimal` the moment a tick arrives, and the shock math works on Decimal. The plan
  schedules the broader Decimal audit later; this slice moved up because the tab was
  displaying a provably wrong number today (deviation 3).

## Step 7 — the board UI: provider and age

**Needed:** the plan text for this phase asks for exactly two board additions — each
quote's provider, and its age measured from the provider's clock. This step adds those two
columns and nothing else; the board otherwise stays as Phase 1 left it.
...
- **The ticket needed two small fixes** (found driving the exit demo):
...
## Step 8 — verification (the exit demo, as evidence)

- **Board over the API** — both symbols with the full quote contract. Captured at 23:14
  Warsaw time; the US market had closed at its normal hour — 16:00 New York, which is
  22:00 Warsaw — 74 minutes before this check. That is why both rows read STALE:

  ```
  FINNHUB AAPL 310.03 LAST REALTIME t=2026-08-18T20:00:00+00:00 STALE stale_after=180
  FINNHUB SPY  767.45 LAST REALTIME t=2026-08-18T20:00:00+00:00 STALE stale_after=45
  ```

  The provider timestamp `20:00 UTC` is exactly 16:00 New York — the real closing-bell
  trade. The Age column counts from that moment.
- **Scheduler behavior:** market status detected `post-market` on boot → 300 s confirmation
  cadence observed in the tick log. Budget after a full cycle: 48/48 tokens,
  `requests_today` counting every call including the status check.
- **Change-only history proven:** many polls of unchanged closing prices → exactly one
  history row per symbol, while the board's `received_at` kept advancing.
- **Refresh endpoint:** `POST /refresh?symbol=AAPL` → 200 with a fresh tick. `NVDA` (not in
  the active set) → 404. Missing symbol → 400. Unknown provider health → 404.
- **The trade:** ticket on the Market Data view → book EQ-DESK → AAPL → backend mark 310.03
  → BUY 10 accepted → trade row ACTIVE with `market_data_provider` NULL — nothing writes
  that column at execution yet, and pricing resolved the provider through its fallback
  constant (Step 6). A manual refresh pushed the next tick and pricing wrote:

  ```
  fair_value 3100.30 · total_pnl 0.0000 · market_data_provider FINNHUB
  market_data_timestamp 2026-08-18 20:00:00+00
  ```
- **Tier promotion observed:** the moment the trade existed, AAPL's threshold tightened
  180 s → 45 s on the next active-set reload.
- **Audit trail:** `BOOK_CREATED`, `TRADE_CREATED`, plus the stream disconnect/reconnect
  pair from a mid-verification rebuild. No provider error events — none occurred.
- **Static + UI:** `py_compile` clean; oxlint, knip, `vite build` clean; Playwright over
  Market Data, Trades, Valuations, System Overview — zero console errors or warnings; 8/8
  containers up, migrations idempotent.

## Deviations from the plan's text

1. **`DEFAULT_QUOTE_PROVIDER` fallback in pricing.** The plan re-keys the cache to
   (provider, symbol) but never says which provider a trade resolves to while nothing
   writes `market_data_provider` on trades. The interim rule is Step 6's constant naming
   the only wired provider.
2. **`BENCHMARK_PROVIDER` landed ahead of its planned phase.** One env var and a one-line
   guard now; without it, alpha/beta would start silently double-sampling SPY the day a
   second provider quotes it (Step 6).
3. **A slice of the planned Decimal work landed early** — see Step 6: the scenario tab was
   shipping a shock that provably did nothing.
4. **Two ticket fixes** (Step 7) — the ticket's rebuild belongs to a later phase by plan,
   but the exit demo runs through today's ticket, so it had to work against the provider
   board now.
...
7. **The demo watchlist row was seeded by SQL** — the watchlist has no add/remove UI or
   endpoint yet. Until it does, a symbol enters the universe via
   `INSERT INTO watchlist_items (symbol, asset_class, currency, created_at)
   VALUES ('AAPL', 'EQUITY', 'USD', now());` — the scheduler picks it up within 15 s. The
   AAPL row and the EQ-DESK book stay in the dev database as the demo fixture.

## Closed during owner review (2026-08-19)

Findings from hands-on review, fixed in place. Most share one root: code written when
...
- **A new trade took minutes to appear in Trades/Books/Valuations — fixed at both ends.**
  ...
  Verified live: open intent → visible in the blotter with fair value in ≤ 4 s; close →
  CLOSED with realized PnL in ≤ 2 s; pricing restart → all trades re-valued within seconds
...
- **Closing one side panel closed the other one too.** With the ticket and a log story
  ...
  directions and both keys.
- **The scheduler was one file doing four jobs — split into four files.** (Owner: too much
  for one file, with more providers coming.) The new layout is Step 4.1's table: the
  ...
  line. Behavior is unchanged — the ops endpoints, refresh, and thresholds were
  re-verified identical after the rebuild, with zero error logs.
- **Comment policy settled.** Docstrings that had drifted toward rationale prose were
  trimmed to one-line contracts. The standing bar, codified in AGENTS.md for every future
  phase: `#` comments serve exactly two purposes — a crucial constraint the code cannot
  ...
  `refresh_symbol` three). Never rationale — all why lives in the docs and this report —
  ...
  carry none; the owner pruned to exactly that line during review.
- **The report itself was re-scoped to the present phase.** Rule from review: a phase
  report describes what exists at the end of its phase — future work is the plan's
  content, and the report may only point at the plan, never retell it. The section below
  replaced a forward-looking "carried forward" list; standing rule for every future
  report.
...
- **Outside US market hours every row reads STALE.** The age is honest — the last trade
  ...
  reopen, so the alarm tone is misleading. Ruled misleading during the review (owner: "is
  closed market really stale?"); the display fix is recorded in the plan.
- **One of six registered providers is wired.** `/providers` lists all six with their
  probed capability facts; five report `wired: false` and have no client, no feed, no
  runtime state.
- **The watchlist has no UI and no endpoint.** A symbol enters by SQL insert
  (deviation 7).
- **Recorded history is not served.** The change-only history table has been filling since
  this phase, but no endpoint reads it — the trend sparkline still shows only the ticks
  the open browser tab has itself witnessed, and forgets them on reload.
- **UNSUPPORTED and MISSING never display.** The board lists only rows that exist, and a
  single provider cannot disagree with itself; both states exist in the classifier, but
  nothing on screen can produce them yet.
- **Provider ops are JSON-only.** Status, budget, market session, and spend today are
  served by `/providers` and `/providers/<name>/health`; no UI screen shows them.
- **The ticket is unchanged by design** beyond Step 7's two fixes: no provider choice, no
  STALE acknowledgement, no wired refresh button, no slippage capture. Trades record no
  provider at execution — `market_data_provider` stays NULL and pricing resolves it by
  fallback (Step 6).
- **The FX pricing branch is dormant.** It still reads the legacy single-provider `spot`
  key; no wired provider serves FX, so the branch cannot run.
- **Curve-priced classes (BOND, IRS, options) stay unpriceable** — no curves exist yet.
- **Valuation writes are unthrottled** — one row per trade per tick, about 4 per minute
  per trade at open-market tier-1 cadence.
