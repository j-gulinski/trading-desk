...
Phase 1 added the contracts and storage sections; the curve/PLN investigation write-up lands
with Phase 5; the full per-provider freshness table lands in the README with Phase 7.
...
2. **Alpha Vantage equities are EOD** (date-only timestamp). Pretending otherwise would be a
   freshness lie → the `quote_grade` concept (D3).
...
10. **NBP has no interest-rate endpoint** (404 verified) → the PLN-curve investigation stands
    as the domain-analysis centerpiece (write-up lands with Phase 5).
...
- **`price_basis`** records which of those cases produced `mid`: `BID_ASK`, `REFERENCE_MID`,
  or `LAST`. Execution semantics (BUY at ask / SELL at bid, D12) attach in Phase 4; valuation
  and display headline always use `mid` (D13).
- **`quote_grade`** says what kind of price this *is*: `REALTIME` (a live tradable quote),
  ...
  The grade travels with every quote so an end-of-day price can never masquerade as live.
...
Thresholds are 2–3× each feed's *scheduled* cadence and arrive with the schedulers (Phases
2–3) as configuration; the classifier is a pure function of (supported, timestamp, now,
threshold). Policy, enforced from Phase 4: MISSING and UNSUPPORTED block the trade; STALE
warns and requires an explicit acknowledgement recorded in the audit trail (D3).
...
actually quotes a given ticker) is computed once at watchlist-add and cached on the
watchlist row (Phase 3).
...
(e.g. 3M), so tenor-matching validation (Phase 5) has a schema fact to check against.
...
- **The symbol master is the `watchlist_items` table** — the user-curated active universe
  ...
  tradeable — scope is a user decision, capped by `MAX_ACTIVE_SYMBOLS` (D4, Phase 3).
- The polled *active set* (watchlist ∪ open-trade symbols ∪ benchmark) is the scheduler's
  concern, arriving in Phase 2.
...
| `market_data_snapshots` | quote history, one row per *changed* quote, with `raw_payload` | append; indexed (provider, symbol, received_at) | ~10k rows/day worst case; retention-swept (Phase 2) |
...
Provenance chain (D2): a trade freezes `market_data_provider`, `entry_price_timestamp`,
`client_seen_price`, and `entry_snapshot_id` — a foreign key to the exact history row used at
execution, one join from any trade to the provider's raw payload. Valuations stamp
...
The `entry_snapshot_id` FK is deliberately strict (no cascade): the Phase 2 retention sweep
must skip snapshot rows referenced by trades, so provenance can never silently vanish —
referenced raw payloads outlive the retention window by design.
