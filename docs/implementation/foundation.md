# Foundation — fork and deep clean

This guide explains the clean foundation of `trading-desk`: no random flows, seeded prices or
generator-specific machinery. It covers the fork boundary, shared runtime, configuration,
container images and the empty-but-honest startup behavior on which provider integrations
depend.

## Foundation decisions

These rules define the policy layer; the numbered sections explain the mechanics. The
README's "Operating decisions" table summarizes the business-facing subset.
...
8. **Documentation is organized by capability, and the README stays the minimal runbook.**
   *(Step 5)*
...
- **Hosting portability is deliberately deferred.** The IPv6
  dual-stack bind Railway's private network would need and the `PORT`/`BIND_HOST` environment
  contract. No speculative hosting enablement lives in the code; future hosting work stays
...
- **Migration availability inside every service image:** every image copies
...
**Needed:** D24 — a knob without a written *why* does not exist — and documentation must
describe the current system rather than its synthetic predecessor.
...
- **`.env.example` is lean:** variables plus one pointer comment; every knob's rationale lives
  ...
  `trading_desk`.
- **Docs cleared to a focused starter set.** The pre-fork topic docs,
  ...
  concise `docs/architecture.md`, `docs/configuration.md`, the implementation roadmap, and
  feature guides under `docs/implementation/`. A feature change updates its guide in the same
  change.
- **`README.md` reset to the minimal runbook** (what the system is, fork context, run steps,
  key signup links, service table, testing policy, docs index). Provider endpoints, price
  basis and freshness live in feature references once running code supports them. `AGENTS.md`
  matches the unified image, runtime and documentation structure.
...
The foundation intentionally stops at these boundaries:
