...
euro-area yield curves and a documented PLN composite — plotted on the Market Data view and
...
| Curve catalog | Five curves, each the latest stored set per name: `USD_TREASURY` (FRED, 11 DGS series), `EUR_GOV_AAA` / `EUR_GOV_ALL` (ECB yield curves), `PLN_REF` (composite from two FRED/OECD anchors, linear interpolation between them, labeled `COMPOSITE_REF` with a declared 3M index tenor), `PLN_NBP_BASE` (flat at the configured NBP reference rate, labeled `POLICY_PROXY`). Points keep the published percent rate with per-point `source_series` + `source_as_of`; NULL series marks a derived point; each set retains its raw source response. | Free tiers publish no WIBOR/Euribor term structures — the honest options are real official curves, a labeled composite, and a labeled proxy, never a curve presented as something it is not. |
| Curve-priced execution | BOND, IRS and EUROPEAN_OPTION open through the same server-priced gate as spot: the ticket previews a model value (pricing `POST /price`), the server recomputes the PV from the stored curves (and the underlying's board row for options), compares it against `client_seen_price` (IRS deviation measured against notional), and freezes each chosen curve's name, provider and as-of into the trade terms. Currency and index-tenor guards reject incompatible choices with the sentence ("a PLN IRS cannot discount on USD_TREASURY — it is a USD curve"). Close recomputes the same model value from the current stored curves. | A model-priced fill is auditable only if the server owns the model inputs; the guards make the projection-vs-discount choice a real, checked decision instead of a free-text field. |
...
| Calendar polling | Reference and curve feeds poll inside each source's publication window (NBP ~11:45–12:20 Warsaw; ECB EXR ~15:55–16:45 and yield curves ~11:55–12:45 Frankfurt; FRED ~16:15–17:30 New York; business days) every 5 min until a new as-of appears, then hourly confirmation polls; the monthly OECD Poland anchors refetch weekly. Keyless sources carry the full provider status machine but no token bucket or ledger; FRED keeps a 108/min bucket from its published 120/min limit. | Fixings and daily curves publish once per business day — polling on a cadence would either hammer the source or miss the window; invented budgets on an unlimited source would be fiction. |
| Official history backfill | When a reference pair has at most one stored observation, the feed backfills up to `REFERENCE_BACKFILL_DAYS` (90) of official fixings after its first live round — NBP range endpoints, ECB `lastNObservations` — as ordinary change-only snapshots: each row's `provider_timestamp` is the fixing's own as-of, `received_at` is the backfill moment, raw is that day's response slice. | The reference drill should show the series the source actually publishes — weeks of daily fixings — while the two-clock contract stays honest about when this application ingested them. |
| Session fields | Quote responses' session data — day open/high/low (both quote providers), 52-week range, volume and average volume (Twelve Data) — is stored as received on the board row as nullable extras, rides every tick, and renders in the Quote Detail session block with an honest n/a where a feed does not publish the field. The board table itself is unchanged; change-only snapshots stay price provenance. | The fields were already paid for in responses the desk fetches anyway; a session block that invents zeros for Finnhub volume would be the UNSUPPORTED lesson unlearned. |
...
| ECB YC → market-data | HTTP polling, publication calendar | every 5 min inside the ~11:55–12:45 Frankfurt window until a new as-of; hourly confirmation otherwise | `EUR_GOV_AAA` and `EUR_GOV_ALL` yield-curve sets (csvdata), one request per curve. |
| FRED → market-data | HTTP polling, publication calendar | every 5 min inside the ~16:15–17:30 New York window until a new as-of; hourly confirmation otherwise; the two OECD Poland anchors weekly | 11 DGS series → `USD_TREASURY`; interbank + gov-bond anchors → the `PLN_REF` composite. |
| NBP proxy curve → market-data | local rebuild on the same loop | hourly; a new set only when the Warsaw date or the configured rate changes | `PLN_NBP_BASE`, flat at `NBP_REFERENCE_RATE_PERCENT` — no HTTP, so a build never masks fixing failures. |
| NBP/ECB history → market-data | one-shot range fetch after the first live round | once per boot, only for pairs with ≤1 stored observation | Backfills up to 90 days of official fixings as change-only snapshots for the reference drill. |
