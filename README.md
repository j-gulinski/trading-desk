...
synthetic feed with real sources: Finnhub (US equities/ETFs), Twelve Data (equities, FX,
metals) and Alpha Vantage (US equity/ETF EOD plus FX) as quote providers, NBP and ECB official FX fixings as reference data feeding
...
Each symbol is watched per provider, so the same asset can be compared across independent feeds and
...
| Polling tiers | Finnhub: tier 1 (open-trade symbols, benchmark) 15 s, tier 2 (remaining watchlist) 60 s. Twelve Data: flat batches whose true cadence is the greater of the 15-min knob and the daily-ledger pace, with per-symbol due-times staggered. Alpha Vantage: US equities once after the session and selected FX at most twice daily. | Each source is paced to its product grade and free entitlement; the strategy line states the computed cadence and next due time. |
...
| Freshness | Five internal states per (provider, symbol). Market open: LIVE until 3× the open cadence on the provider clock, then STALE. Market closed: internal CLOSED while confirmation polls keep arriving, then STALE. Every usable closing mark renders neutrally as `EOD (date)`; Alpha equity is never presented as LIVE. MISSING = no data yet; UNSUPPORTED = a capability fact. | STALE means "this feed should be updating and is not"; one EOD presentation keeps all usable closing marks consistent without an intraday claim. |
...
| Provider failure | Provider responses drive a per-provider state machine: 429/body-level throttle → RATE_LIMITED; 401/403 → AUTH_FAILED; network/5xx → ERROR. Every external call writes minimal success/failure/rate-limit audit metadata without a response body; state transitions remain separately audited. Per-symbol data errors do not change provider state. | Failures are visible and scoped without putting vendor payloads or credentials in the business audit. |
...
| Quote audit volume | `QUOTE_WRITTEN` is audited whenever a stored quote changes; unchanged confirmation polls update the board clock without another snapshot or quote audit. | The audit follows economic changes, not polling activity. |
...
| Portfolio summary | One shared browser aggregation feeds Business Overview, Books and Valuations: open gross entry value (`abs(quantity) × entry price × multiplier`), unrealized, realized and total PnL, plus open/closed counts. Per-currency subtotals remain primary; a selected reporting currency supplies the converted total. | The three screens cannot drift, closed positions do not remain in open capital, and unlike currencies are never added without an explicit FX path. |
...
[Alpha Vantage](https://www.alphavantage.co/support/#api-key), plus
[fred.stlouisfed.org](https://fredaccount.stlouisfed.org/apikeys). NBP, ECB
...
| Alpha Vantage → market-data | HTTP polling, persisted daily ledger | equities once after the US session; FX at most every 12 h; ≥15 s between any two calls; 22 of 25 calls/day | `GLOBAL_QUOTE` US EOD marks and `CURRENCY_EXCHANGE_RATE` bid/ask FX. The ledger and fresh due-times survive restart. |
| Finnhub + Twelve Data → market-data | HTTP search calls | on watchlist search, cached 10 min per query | Symbol discovery; existing normalized US equity/ETF and FX identities attach Alpha capability without spending an Alpha typeahead call. |
...
| FRED → market-data | scheduled HTTP polling, budget-reserved builder | US government bonds every six hours; 15-minute failure retry | 11 DGS series → USD · Government bonds. |
...
| trade detail ← market-data curve history | REST `GET /market-data/curves/<provider>/<curve>/<as-of>` | once when a bond detail opens | Retained entry curve points compared with the live curve; the drawer shows the maturity-rate shift in basis points beside the bond repricing and PnL. |
...
| ticket ← trade-action term schemas | REST polling | every 5 s while the New trade panel is open | Term fields plus the curve catalog behind the ticket's curve pickers; IRS receives one combined discount/projection risk-free choice. |
...
| pricing → Postgres; → blotter, browser | sampled valuation insert + live SSE valuation stream | SSE on every revaluation; persistence at most once per trade per 60 s, plus terminal close | Live display stays responsive while `valuations` growth is bounded; each persisted sample writes `VALUATION_UPDATED`, missing inputs write deduplicated `VALUATION_BLOCKED`. |
...
No unit-test suite by design: the final integrated workflow is retained in
`scenarios/full-provider-flow.http`; focused provider behavior remains in
