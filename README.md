...
either membership can be removed independently. The Market Data board shows separate Bid,
Ask and Last fields, a previous-close-anchored Today trend, and market-aware
...
| Request budget | Client-side cap at ~80% of each configured allowance (Finnhub: 48 of 60 req/min; Twelve Data: 6 of 8 credits/min **and** 640 of 800 credits/day, spent no faster than the elapsed fraction of the UTC day). Quote, market-status, search and manual-refresh calls all consume the budget that applies to their provider. | The margin absorbs window misalignment and retries; the daily governor spreads a scarce budget across the whole day instead of burning out by noon. |
...
| Trend history | The board starts at provider `previous_close`, includes today's locally stored price changes in five-minute buckets, and ends at the current board value. | Change and Trend use the same endpoints, including after the market closes; no vendor backfill is implied. |
...
| Registration | The watchlist stores a **provider choice per symbol**, not a capability. Search results list the capable providers as toggles and the Add button counts what it will create; adding a provider to a watched symbol merges. `DELETE /watchlist/<symbol>?provider=` drops one feed and leaves the others ticking, answering `{removed_providers, remaining_providers, still_polled}`; omitting the parameter removes the symbol. Adding or removing reloads every feed's active set immediately, and a `market_remove` event names the (provider, symbol) rows that left so open tabs drop those and only those. Rows kept alive by an open position or the benchmark carry a POS/BMK tag instead of the remove control. | Each feed must be addable and removable independently for valid side-by-side comparison. On a 640-credit/day budget, polling an unselected provider wastes scarce capacity. |
...
| Twelve Data → market-data | HTTP polling, batched (≤6 symbols/call) | 15 min best case; the daily ledger (640 credits vs elapsed day fraction) is the real pace | Quote ingestion for equities, FX, metals — one credit per symbol per batch. |
...
`scenarios/provider-trading.http` (any REST-client runner), with browser verification for
the interactive watchlist, ticket and provider-log paths.
